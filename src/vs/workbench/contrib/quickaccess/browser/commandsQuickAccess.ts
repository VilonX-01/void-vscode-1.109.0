...
export class CommandsQuickAccessProvider extends AbstractEditorCommandsQuickAccessProvider {
	...
	protected async getAdditionalCommandPicks(allPicks: ICommandQuickPick[], picksSoFar: ICommandQuickPick[], filter: string, token: CancellationToken): Promise<Array<ICommandQuickPick | IQuickPickSeparator>> {
		...
		// Void fork: do not offer built-in "Ask in Chat" from Command Palette.
	...
	private getGlobalCommandPicks(): ICommandQuickPick[] {
		...
		for (const action of globalCommandsMenuActions) {
			if (this.shouldHideBuiltinChatCommand(action.item.id)) {
				continue;
			}
	...
	private shouldHideBuiltinChatCommand(commandId: string): boolean {
		const id = commandId.toLowerCase();
		return id.startsWith('workbench.action.chat')
			|| id.startsWith('workbench.action.quickchat')
