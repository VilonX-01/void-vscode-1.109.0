...
export class ToolsService implements IToolsService {
	...
	constructor(
		...
		const isURIStreaming = (uri: URI) => {
			const diffAreas = editCodeService.diffAreasOfURI[uri.fsPath];
			if (!diffAreas) return false;
			for (const diffareaid of diffAreas) {
				const diffArea = editCodeService.diffAreaOfId[diffareaid];
